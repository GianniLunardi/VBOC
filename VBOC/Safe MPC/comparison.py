...
use_data = 'softterm' # softterm, hardterm, receidinghard, receidingsoft

if use_data == 'softterm':
    data_vboc = np.load(data_dir + "results_softterm.npz")
if use_data == 'hardterm':
...
plt.plot(np.linspace(0, len(times_no), len(times_no)), np.ones(len(times_no)) * np.quantile(times_no, 0.99),
         label="99% quantile naive MPC", color='fuchsia', linestyle='--')
plt.plot(np.linspace(0, len(times_hard), len(times_hard)), np.ones(len(times_hard)) * np.quantile(times_hard, 0.97),
         label="97% quantile MPC + VBOC", color='DarkBlue', linestyle='--')
