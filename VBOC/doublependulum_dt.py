...
class OCPdoublependulum:
    def __init__(self):
        ...
        self.ocp.model.cost_expr_ext_cost_0 = w1 * dtheta1 + w2 * dtheta2  
        ...
        self.ocp.constraints.lbx_e = np.array(
            [self.thetamin, self.thetamin, 0., 0.]
        ) 
        self.ocp.constraints.ubx_e = np.array(
            [self.thetamax, self.thetamax, 0., 0.]
        )
        ...
        self.ocp.constraints.C = np.zeros((2,4))
...
class OCPdoublependulumINIT(OCPdoublependulum):
    ...
    def OCP_solve(self, x_sol_guess, u_sol_guess, p, q_init):
        ...
        for i in range(self.N):
            ...
            # self.ocp_solver.constraints_set(i, 'C', np.zeros((2,4)))
        ...
        d = np.array([p.tolist()])
        dt = np.transpose(d)
        C[:,2:] = np.identity(2)-np.matmul(dt,d)
        self.ocp_solver.constraints_set(0, "C", C, api='new') 

        q_init_lb = np.hstack([q_init, [-self.dthetamax] * 2])
        q_init_ub = np.hstack([q_init, [self.dthetamax] * 2])
